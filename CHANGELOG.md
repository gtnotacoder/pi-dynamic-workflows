...
- Artifact-first `/deep-research`: primary-source-first policy; bounded question, angles, `minSupport`, fan-in, recursively strict schemas, at most 3 supported claims, and a 120-character summary keep every prompt/result below 10KB. Agents use read-only repo tools plus `web_search`/`web_fetch`; none receives `write`. Only URLs actually gathered/fetched may survive verification; empty pages do not consume evidence slots. The host accepts only HTTP(S) citations, renders a cited report into a fresh private OS temporary directory, and delivers a compact acknowledgement.
