...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  ...
  const buildPromptGuidelines = (): string[] =>
    [
      ...
      "For workflow, available globals are agent(prompt, opts), parallel(thunks), pipeline(items, ...stages), dag(nodes), phase(title), log(message), args, cwd, process.cwd(), budget, and checkpoint(promptText, options?) for a deterministic, journaled human-decision gate. Every workflow must call agent() at least once; do not use workflow only to declare phases or return a static object.",
      ...
      "For workflow, separate facts from decisions: facts include tool/file results and time-scoped observations such as current CI status — always cite the command or source and observation context. Decisions are human choices: preferences, approvals, policy/routing tradeoffs. Never classify CI pass/fail as a decision. This refines 'Ground Claims in Evidence' for workflow authors.",
      "For workflow, human-in-the-loop is AFK-safe and never silently approves a decision: gather all facts autonomously BEFORE the decision gate, then gate a consequential human decision on checkpoint(). A background/detached run has no human present, so checkpoint() replays its journaled default — therefore every consequential checkpoint MUST pass an explicit conservative default (normally `default: false` for any ship/merge/delete/proceed gate) or `headless: 'abort'` so an AFK run stops or declines instead of proceeding as approved. Do NOT rely on the implicit `default ?? true`. Recorded human choices are immutable constraints, not inferred facts — never re-derive or override them later in the run. Do NOT block on undefined human input or spin-poll.",
      "For workflow, expand-contract means: EXPAND the new API/form beside the old, MIGRATE callers in independently green bounded batches, then CONTRACT/delete the old form. It never means propose candidates, grill one, arbitrary 10+ file thresholds, or compaction policy. Add this as a planning exception for wide mechanical refactors only; ordinary feature/bug work stays thin vertical steps. Dependency ordering must keep CI green.",
      "For workflow, avoid tautological tests: the expected value must come from an independent source of truth (a spec, a known constant, a hand-computed oracle), never recomputed the same way as the code under test. This is guidance for when tests are written, not a requirement to write tests first.",
